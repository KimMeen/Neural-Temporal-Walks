...
def preprocess(data_name, node_edge_feat_dim):
    ...
    with open(data_name) as f:
        ...
        for idx, line in tqdm(enumerate(f)):
            ...
            feat = np.zeros(node_edge_feat_dim)
...
def run(args):
    ...
    df, feat = preprocess(PATH, node_edge_feat_dim)
    ...
    new_df.to_csv(OUT_DF, index=True)
    np.save(OUT_FEAT, feat)
    np.save(OUT_NODE_FEAT, rand_feat)


parser = argparse.ArgumentParser('The script to preprocess csv source data for NeurTWs framework')
parser.add_argument('--dataset', choices = ['CollegeMsg'], help='dataset name')
